...
static char     s_line[VITALS_LINES][VITALS_COLS];
static int      s_nlines;
...
void vitals_begin(void)
{
    ...
    s_nlines = vitals_verdict(&s_prev, now == ESP_RST_POWERON, s_line,
                              VITALS_LINES);
...
{
    /* A mark that this run began, and in which mode - read by the next boot so
     * a run that wrote nothing else is not described by an older one. One
     * write a boot. */
    write_record(usb_mode, false, 0, false);
}
