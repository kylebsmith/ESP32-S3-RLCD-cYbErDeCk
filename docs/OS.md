...
| EP0 + 5 IN and 5 OUT `[MEASURED]` | Read from the installed ESP-IDF: `components/soc/esp32s3/include/soc/usb_dwc_struct.h` declares `in_eps[6]` and `out_eps[6]`, and those arrays **include EP0** — so five usable each way, not six. CDC-ACM costs 2 IN + 1 OUT (notification, bulk in, bulk out) and USB MIDI costs 1 IN + 1 OUT, so a **CDC + MIDI composite fits with two IN and three OUT to spare**. This entry was tagged `[FACT]` and attributed to HARDWARE.md, which contains no endpoint budget and mentions USB once, about a current sink; it was retagged `[OPEN]` and is now closed against a citable file rather than a recollection. |
